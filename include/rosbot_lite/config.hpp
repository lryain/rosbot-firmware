#pragma once

#include <Arduino.h>

#include "battery_adc.hpp"
#include "communication_manager.hpp"
#include "eeprom.hpp"
#include "fan.hpp"
#include "hardware_encoder.hpp"
#include "imu_bno055.hpp"
#include "led_indicator.hpp"
#include "led_strip.hpp"
#include "motor_array.hpp"
#include "motor_drv8874.hpp"
#include "ntc.hpp"
#include "pid.hpp"
#include "ros/publishers/battery_publisher.hpp"
#include "ros/publishers/buttons_publisher.hpp"
#include "ros/publishers/imu_publisher.hpp"
#include "ros/publishers/joint_state_publisher.hpp"
#include "ros/publishers/motor_current_publisher.hpp"
#include "ros/publishers/motor_stall_publisher.hpp"
#include "transport/spi_transport.hpp"

inline constexpr uint32_t ADC_MAX_VALUE = 1023;
// 空闲IO PD14 PD15
inline constexpr uint8_t AUDIO_SHDN = PD15; // PE10
inline constexpr uint8_t AUDIO_DAC_OUT = PA4;
inline constexpr uint8_t CAN1_RX_PIN = PD0;
inline constexpr uint8_t CAN1_TX_PIN = PD1;
inline constexpr uint8_t SBUS_RX_PIN = PD2;
inline constexpr uint8_t EN_LOC_5V = PE2; // Power enable pin for peripherals (PERI_5V, etc.)
// IMU I2C2
inline constexpr uint8_t I2C_SDA = PB11;
inline constexpr uint8_t I2C_SCL = PB10;
inline TwoWire i2c(I2C_SDA, I2C_SCL);
// I2C1 FOR WP5 Power Board
inline constexpr uint8_t I2C1_SDA = PB7;
inline constexpr uint8_t I2C1_SCL = PB6;
inline TwoWire ext_i2c(I2C1_SDA, I2C1_SCL);

inline constexpr uint8_t EXT_PA8 = PA8;         // EXT_GPIO6_RADAR/PA8
inline constexpr uint8_t EXT_PA0 = PA0;         // UART4_TX/PA0
inline constexpr uint8_t EXT_PA1 = PA1;         // UART4_RX/PA1
inline constexpr uint8_t EXT_PC14 = PC14;       // 
inline constexpr uint8_t EXT_PC15 = PC15;       //
inline constexpr uint8_t EXT_PD14 = PD14;       // EXT_GPIO4/PD14
inline constexpr uint8_t EXT_PD15 = PD15;       // EXT_GPIO5/PD15
// USART2 PD6 PD5 程序下载串口，有待验证
inline constexpr uint8_t PB_SERIAL_RX = PA1;    // PD6
inline constexpr uint8_t PB_SERIAL_TX = PA0;    // PD5
inline HardwareSerial pb_serial(PB_SERIAL_RX, PB_SERIAL_TX);
// 诊断串口，连接到 SBC 的 USART3
inline constexpr uint8_t DIAG_SERIAL_RX = PD9;
inline constexpr uint8_t DIAG_SERIAL_TX = PD8;
inline HardwareSerial diag_serial(DIAG_SERIAL_RX, DIAG_SERIAL_TX);

inline constexpr uint8_t PB_SHD_DETECT = PD4;
inline constexpr uint8_t PB_SHD_CONFIRM = PD7;
inline constexpr uint32_t SHUTDOWN_WAIT_MS = 5000;
// BAT_ADC/PA5
inline constexpr BatteryAdcConfig battery_adc_config = {
    .adc_pin = PA5,
    .v_ref = 3.3f,
    .v_min = 9.6f,
    .v_max = 12.6f,
    .divider = (5.6e4 + 1.0e4) / 1.0e4,
    .correction = 0.986f,
    .adc_resolution_scale = 1.0f / ADC_MAX_VALUE,
};

inline constexpr uint8_t PUSH_BUTTON1 = PE14;
inline constexpr uint8_t PUSH_BUTTON2 = PE15;
// I2C2
inline constexpr EepromConfig eeprom_config = {
    .i2c_bus = i2c,
    .dev_id = 0x50,
    .page_size = 16,
    .write_delay_ms = 5,
};
inline Eeprom eeprom(eeprom_config);

inline BoardRevisionConfig board_revision_config = {
    .eeprom = eeprom,
    .block = 0x00,
    .addr = 0x00,
    .max_length = 4,
    .retry_count = 3,
};

inline constexpr float GEAR_RATIO = 50.0f;
inline constexpr uint16_t ENCODER_CPR = 64;
inline constexpr float TICKS_PER_REVOLUTION = ENCODER_CPR * GEAR_RATIO;
inline constexpr float RAD_PER_TICK = (2.0f * PI) / TICKS_PER_REVOLUTION;
inline constexpr float LOW_PASS_ALPHA = 0.05f;

inline constexpr HardwareEncoderConfig enc_fl_config = {
    .pin_a = PD12,
    .pin_b = PD13,
    .timer = TIM4,
    .inv_dir = true,
    .rad_per_tick = RAD_PER_TICK,
    .frame_id = "fl_wheel_joint",
    .alpha = LOW_PASS_ALPHA,
};

inline constexpr HardwareEncoderConfig enc_fr_config = {
    .pin_a = PB4,
    .pin_b = PB5,
    .timer = TIM3,
    .inv_dir = false,
    .rad_per_tick = RAD_PER_TICK,
    .frame_id = "fr_wheel_joint",
    .alpha = LOW_PASS_ALPHA,
};

inline constexpr HardwareEncoderConfig enc_rl_config = {
    .pin_a = PA15,
    .pin_b = PB3,
    .timer = TIM2,
    .inv_dir = true,
    .rad_per_tick = RAD_PER_TICK,
    .frame_id = "rl_wheel_joint",
    .alpha = LOW_PASS_ALPHA,
};

inline constexpr HardwareEncoderConfig enc_rr_config = {
    .pin_a = PE9,
    .pin_b = PE11,
    .timer = TIM1,
    .inv_dir = false,
    .rad_per_tick = RAD_PER_TICK,
    .frame_id = "rr_wheel_joint",
    .alpha = LOW_PASS_ALPHA,
};

inline constexpr NtcConfig ntc_cfg{
    .pin = PB1,
    .pullup_resistance = 5230.0f,
    .c1 = 1.112613927e-03f,
    .c2 = 2.37277392e-04f,
    .c3 = 7.1670e-08f,
    .offset = 273.15 + 3,
    .adc_max = 1023.0f,
};
inline Ntc ntc(ntc_cfg);

inline constexpr uint8_t FAN_PWM_PIN = PB0;
inline constexpr FanConfig fan_config{
    .pin = FAN_PWM_PIN,
    .timer_instance = TIM8,  // ← jawnie, bez auto-detekcji
    .timer_channel = 2,      // CH2 — library wykryje CH2N z PinMap
    .gpio_af = GPIO_AF3_TIM8,
    .complementary = true,  // CH2N = odwrócony sygnał
    .mode = FanMode::Proportional,
    .pwm_frequency = 25000,  // 25 kHz — standard PC fan
    .temp_low = 30,          // poniżej 30°C — duty_min
    .temp_high = 55,         // powyżej 55°C — duty_max
    .duty_min = 20,          // 20% — minimalne obroty (żeby fan nie zatkał)
    .duty_max = 100          // 100% — full blast
};

inline constexpr ImuBno055Config imu_bno055_config = {
    .bus = &i2c,
    .i2c_addr = 0x29,
    .sensor_id = 0x37,
    .int_pin = PD3,
    .axis_config = Adafruit_BNO055::REMAP_CONFIG_P1,
    .axis_sign = Adafruit_BNO055::REMAP_SIGN_P2,
};

inline constexpr uint8_t RED_LED = PE4;
inline constexpr uint8_t GRN_LED = PE3;
inline constexpr LedIndicatorConfig led_status_config = {
    .pin = RED_LED,
    .initial_state = HIGH,
    .blink_period_ms = 500,
    .label = "STATUS",
};

inline constexpr uint16_t IDLE_ANIMATION_CHANGE_MS = 3500;
inline constexpr uint16_t IDLE_ANIMATION_INTERVAL_MS = 35;
inline constexpr uint16_t LED_STRIP_TIMEOUT_MS = 1000;

inline constexpr SpiTransportConfig spi_config = {
    .mosi_pin = PC12,
    .miso_pin = PC11,
    .sck_pin = PC10,
    .spi_speed = 4000000,
    .bit_order = MSBFIRST,
    .spi_mode = SPI_MODE3,
};

inline constexpr SwapPair swaps[] = {
    {13, 17},
    {14, 16},
};
inline constexpr LedStripConfig strip_config = {
    .num_leds = 18,
    .swaps = swaps,
    .swap_count = sizeof(swaps) / sizeof(swaps[0]),
    .init_r = 0x0F,
    .init_g = 0x00,
    .init_b = 0x00,
};

inline constexpr uint32_t MOTOR_PWM_FREQ = 20000;
inline constexpr float MAX_VELOCITY = 22.0f;
inline constexpr float MIN_VELOCITY = 0.5f;

inline constexpr float IPROPI_R_SENSE = 560.0f;
inline constexpr float IPROPI_K = 1200.0f;
inline constexpr float IPROPI_V_REF = 3.3f;
inline constexpr float IPROPI_ADC_MAX = 1023.0f;
inline constexpr float IPROPI_SCALE =
    IPROPI_V_REF / IPROPI_ADC_MAX / (IPROPI_R_SENSE / IPROPI_K);

inline constexpr uint8_t IPROPI_FL = PC4;
inline constexpr uint8_t IPROPI_FR = PC5;
// IPROPI_RL / IPROPI_RR remapped from PA3/PA2 -> PC1/PC0 to free PA2/PA3
inline constexpr uint8_t IPROPI_RL = PC0;
inline constexpr uint8_t IPROPI_RR = PC1;

// SERVO_PWM4/PA3 or WS2812 (one-wire) data pin: freed PA3 is used for LED data output
inline constexpr uint8_t WS2812_PIN = PA3;

inline constexpr float IPROPI_OC_MAX_CURRENT = 5.0f;
inline constexpr float IPROPI_STALL_CURRENT = 3.5f;
inline constexpr float IPROPI_STALL_VEL = 0.15f;
inline constexpr uint32_t IPROPI_STALL_MS = 250;
// NSLEEP_L/PD10 NFAULT_L/PE0 NSLEEP_R/PD11 NFAULT_R/PE1
inline constexpr DriverGroupConfig left_motors_driver = {PD10, PE0};
inline constexpr DriverGroupConfig right_motors_driver = {PD11, PE1};
inline constexpr DriverGroupConfig driver_groups[] = {
    left_motors_driver,
    right_motors_driver,
};

inline constexpr MotorDrv8874Config motor_fl_config = {
    .in1_pin = PA6,
    .in2_pin = PA7,
    // PA6 PRIMARY→TIM3_CH1 (conflicts with enc_fr TIM3 encoder).
    // PA7 PRIMARY→TIM1_CH1N complementary (conflicts with enc_rr TIM1 encoder).
    // Force ALT mappings to use free single-channel timers instead.
    .in1_pname = PA_6_ALT1,  // → TIM13_CH1 (AF9, free)
    .in2_pname = PA_7_ALT3,  // → TIM14_CH1 (AF9, free)
    .inv_dir = true,
    .max_velocity = MAX_VELOCITY,
    .min_velocity = MIN_VELOCITY,
    .pwm_freq = MOTOR_PWM_FREQ,
    .frame_id = "fl_wheel_joint",
    .ipropi_pin = IPROPI_FL,
    .ipropi_scale = IPROPI_SCALE,
    .max_current = IPROPI_OC_MAX_CURRENT,
    .stall_current = IPROPI_STALL_CURRENT,
    .stall_vel = IPROPI_STALL_VEL,
    .stall_ms = IPROPI_STALL_MS,
};

inline constexpr MotorDrv8874Config motor_fr_config = {
    .in1_pin = PB8,
    .in2_pin = PB9,
    // PB8/PB9 PRIMARY→TIM4_CH3/CH4 (conflicts with enc_fl TIM4 encoder).
    // Force ALT mappings to use free single-channel timers instead.
    .in1_pname = PB_8_ALT1,  // → TIM10_CH1 (AF3, free)
    .in2_pname = PB_9_ALT1,  // → TIM11_CH1 (AF3, free)
    .inv_dir = false,
    .max_velocity = MAX_VELOCITY,
    .min_velocity = MIN_VELOCITY,
    .pwm_freq = MOTOR_PWM_FREQ,
    .frame_id = "fr_wheel_joint",
    .ipropi_pin = IPROPI_FR,
    .ipropi_scale = IPROPI_SCALE,
    .max_current = IPROPI_OC_MAX_CURRENT,
    .stall_current = IPROPI_STALL_CURRENT,
    .stall_vel = IPROPI_STALL_VEL,
    .stall_ms = IPROPI_STALL_MS,
};

inline constexpr MotorDrv8874Config motor_rl_config = {
    .in1_pin = PE5,
    .in2_pin = PE6,
    .inv_dir = true,
    .max_velocity = MAX_VELOCITY,
    .min_velocity = MIN_VELOCITY,
    .pwm_freq = MOTOR_PWM_FREQ,
    .frame_id = "rl_wheel_joint",
    .ipropi_pin = IPROPI_RL,
    .ipropi_scale = IPROPI_SCALE,
    .max_current = IPROPI_OC_MAX_CURRENT,
    .stall_current = IPROPI_STALL_CURRENT,
    .stall_vel = IPROPI_STALL_VEL,
    .stall_ms = IPROPI_STALL_MS,
};

inline constexpr MotorDrv8874Config motor_rr_config = {
    .in1_pin = PC8,
    .in2_pin = PC9,
    // PC8/PC9 PRIMARY→TIM3_CH3/CH4 (conflicts with enc_fr TIM3 encoder).
    // Force ALT mappings to TIM8_CH3/CH4 (AF3). TIM8 is shared with the fan
    // (CH2N on PB0), but both use 20 kHz so PSC/ARR are identical.
    .in1_pname = PC_8_ALT1,  // → TIM8_CH3 (AF3)
    .in2_pname = PC_9_ALT1,  // → TIM8_CH4 (AF3)
    .inv_dir = false,
    .max_velocity = MAX_VELOCITY,
    .min_velocity = MIN_VELOCITY,
    .pwm_freq = MOTOR_PWM_FREQ,
    .frame_id = "rr_wheel_joint",
    .ipropi_pin = IPROPI_RR,
    .ipropi_scale = IPROPI_SCALE,
    .max_current = IPROPI_OC_MAX_CURRENT,
    .stall_current = IPROPI_STALL_CURRENT,
    .stall_vel = IPROPI_STALL_VEL,
    .stall_ms = IPROPI_STALL_MS,
};

inline constexpr PIDConfig pid_config = {
    .kp = 0.15f,
    .ki = 0.55f,
    .kd = 0.007f,
    .min_output = -1.0f,
    .max_output = 1.0f,
};

inline constexpr const char* NODE_NAME = "rosbot_lite_mcu";
inline constexpr uint16_t DOMAIN_ID = 255;
inline constexpr uint32_t SPIN_TIME_MS = 1;
inline constexpr uint32_t TIMER_MS = 10;
inline constexpr uint32_t PING_TIMEOUT_MS = 50;
inline constexpr uint8_t PING_ATTEMPTS = 4;

inline QueueHandle_t battery_queue;
inline QueueHandle_t imu_queue;
inline QueueHandle_t joint_state_queue;
inline QueueHandle_t led_strip_queue;
inline QueueHandle_t motor_current_queue;
inline QueueHandle_t motor_stall_queue;

inline constexpr uint8_t BATTERY_NUM_CELLS = 3;
inline constexpr float BATTERY_CELL_CAPACITY = 2.6f;
inline constexpr float BATTERY_DESIGN_CAPACITY =
    BATTERY_NUM_CELLS * BATTERY_CELL_CAPACITY;

inline constexpr BatteryPublisherConfig battery_pub_config = {
    .topic = "battery",
    .queue = battery_queue,
    .frame_id = "base_link",
    .design_capacity = BATTERY_DESIGN_CAPACITY,
    .num_cells = BATTERY_NUM_CELLS,
};

inline constexpr uint8_t buttons_pins[] = {PUSH_BUTTON1, PUSH_BUTTON2};
inline constexpr ButtonsPublisherConfig buttons_pub_config = {
    .topic = "buttons",
    .pins = buttons_pins,
    .num_buttons = sizeof(buttons_pins) / sizeof(buttons_pins[0]),
};

inline constexpr ImuPublisherConfig imu_pub_config = {
    .topic = "_imu/data",
    .queue = imu_queue,
    .frame_id = "imu_link",
};

inline constexpr JointStatePublisherConfig joint_state_pub_config = {
    .topic = "_motors/feedback",
    .queue = joint_state_queue,
    .frame_id = "base_link",
};

inline constexpr MotorCurrentPublisherConfig motor_current_pub_config = {
    .topic = "_motors/current",
    .queue = motor_current_queue,
};

inline constexpr MotorStallPublisherConfig motor_stall_pub_config = {
    .topic = "_motors/stall",
    .queue = motor_stall_queue,
};

inline constexpr SerialConfig SBC_SERIAL_CONFIG = {
    .serial = &Serial1,
    .baudrate = 921600,
    .rxPin = PA10,
    .txPin = PA9,
    .timeout_ms = 1,
    .name = "SBC_SERIAL",
};

inline constexpr SerialConfig DIAGNOSTIC_SERIAL_CONFIG = {
    .serial = &diag_serial,
    .baudrate = 921600,
    .rxPin = DIAG_SERIAL_RX,
    .txPin = DIAG_SERIAL_TX,
    .timeout_ms = 1,
    .name = "USART3_DIAG",
};

