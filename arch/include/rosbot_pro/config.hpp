// Copyright 2024 ROSbot PRO
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#pragma once

#include <Arduino.h>
#include <STM32Ethernet.h>

#include "battery_adc.hpp"
#include "communication_manager.hpp"
#ifdef USE_WITTYPI5_POWER
#include "power_board_wp5.hpp"
#endif
#include "eeprom.hpp"
#include "fan.hpp"
#include "hardware_encoder.hpp"
#include "imu_bno055.hpp"
#include "led_indicator.hpp"
#include "led_strip.hpp"
#include "motor_array.hpp"
#include "motor_drv8874.hpp"
#include "ntc.hpp"
#include "pid.hpp"
#include "ros/publishers/battery_publisher.hpp"
#include "ros/publishers/buttons_publisher.hpp"
#include "ros/publishers/imu_publisher.hpp"
#include "ros/publishers/joint_state_publisher.hpp"
#include "ros/publishers/motor_current_publisher.hpp"
#include "ros/publishers/motor_stall_publisher.hpp"
#include "transport/spi_transport.hpp"

// ───────── Arduino settings ─────────
inline constexpr uint32_t ADC_MAX_VALUE =
    1023;  // 10-bit ADC resolution (0-1023)

// ───────── Board Peripherals ─────────
inline constexpr uint8_t AUDIO_SHDN = PD2;
inline constexpr uint8_t AUDIO_DAC_OUT = PA4;
inline constexpr uint8_t EN_LOC_5V = PF13;

// I2C2 (PF0/PF1) — IMU + EEPROM  [ZET6: PF0=I2C2_SDA AF4, PF1=I2C2_SCL AF4]
inline constexpr uint8_t I2C_SDA = PF0;
inline constexpr uint8_t I2C_SCL = PF1;
inline TwoWire i2c(I2C_SDA, I2C_SCL);

// Power board UART — reserved for future use
inline constexpr uint8_t PB_SERIAL_RX = PD6;
inline constexpr uint8_t PB_SERIAL_TX = PD5;
inline HardwareSerial pb_serial(PB_SERIAL_RX, PB_SERIAL_TX);

// ───────── Witty Pi 5 Power Board (I2C1: PB8/PB9) ─────────
// ZET6: PB8=I2C1_SCL AF4, PB9=I2C1_SDA AF4 (dedicated I2C bus, separate from IMU)
// Shutdown pins stay identical to ROSbot XL and are always reserved.
inline constexpr uint8_t PB_SHD_DETECT = PD4;  // power board shutdown detect
inline constexpr uint8_t PB_SHD_CONFIRM = PD7;  // power board shutdown confirm
inline constexpr uint32_t SHUTDOWN_WAIT_MS = 5000;

#ifdef USE_WITTYPI5_POWER
inline constexpr uint8_t WP5_I2C_SDA = PB9;  // I2C1_SDA (AF4)
inline constexpr uint8_t WP5_I2C_SCL = PB8;  // I2C1_SCL (AF4)
inline TwoWire wp5_i2c(WP5_I2C_SDA, WP5_I2C_SCL);

inline PowerBoardWP5Config wp5_config = {
    .i2c_bus = wp5_i2c,
    .i2c_addr = 0x51,
    .v_min = 9.6f,
    .v_max = 12.6f,
    .shd_detect_pin = PB_SHD_DETECT,
    .heartbeat_interval_ms = 30000,
    .low_voltage = 96,        // 9.6V
    .recovery_voltage = 105,  // 10.5V
    .watchdog_count = 3,
    .default_on_delay = 3,
    .power_cut_delay = 20,
    .ps_priority = 1,  // VIN priority
};
#endif

// ───────── Battery (ADC method — no power board in V1) ─────────
inline constexpr BatteryAdcConfig battery_adc_config = {
    .adc_pin = PA5,
    .v_ref = 3.3f,
    .v_min = 9.6f,
    .v_max = 12.6f,
    .divider = (5.6e4 + 1.0e4) / 1.0e4,  // 56k + 10k divider → 6.6
    .correction = 0.986f,
    .adc_resolution_scale = 1.0f / ADC_MAX_VALUE,
};

// ───────── Buttons ─────────
inline constexpr uint8_t PUSH_BUTTON1 = PF11;  // ZET6: PF11 (GPIO)
inline constexpr uint8_t PUSH_BUTTON2 = PF12;  // ZET6: PF12 (GPIO, MCU reset)

// ───────── EEPROM ─────────
inline constexpr EepromConfig eeprom_config = {
    .i2c_bus = i2c,
    .dev_id = 0x50,
    .page_size = 16,
    .write_delay_ms = 5,
};
inline Eeprom eeprom(eeprom_config);

inline BoardRevisionConfig board_revision_config = {
    .eeprom = eeprom,
    .block = 0x00,
    .addr = 0x00,
    .max_length = 4,
    .retry_count = 3,
};

// ───────── Encoders ─────────
// Same as ROSbot XL: 64 CPR, 50:1 gear ratio
inline constexpr float GEAR_RATIO = 50.0f;
inline constexpr uint16_t ENCODER_CPR = 64;
inline constexpr float TICKS_PER_REVOLUTION = ENCODER_CPR * GEAR_RATIO;
inline constexpr float RAD_PER_TICK = (2.0f * PI) / TICKS_PER_REVOLUTION;
inline constexpr float LOW_PASS_ALPHA = 0.05f;

inline constexpr HardwareEncoderConfig enc_fl_config = {
    .pin_a = PD12,
    .pin_b = PD13,
    .timer = TIM4,
    .inv_dir = true,
    .rad_per_tick = RAD_PER_TICK,
    .frame_id = "fl_wheel_joint",
    .alpha = LOW_PASS_ALPHA,
};

inline constexpr HardwareEncoderConfig enc_fr_config = {
    .pin_a = PB4,  // TIM3_CH1 AF2 (PC6 occupied by Motor RR TIM8_CH1)
    .pin_b = PB5,  // TIM3_CH2 AF2
    .timer = TIM3,
    .inv_dir = false,
    .rad_per_tick = RAD_PER_TICK,
    .frame_id = "fr_wheel_joint",
    .alpha = LOW_PASS_ALPHA,
};

inline constexpr HardwareEncoderConfig enc_rl_config = {
    .pin_a = PA15,
    .pin_b = PB3,
    .timer = TIM2,
    .inv_dir = true,
    .rad_per_tick = RAD_PER_TICK,
    .frame_id = "rl_wheel_joint",
    .alpha = LOW_PASS_ALPHA,
};

inline constexpr HardwareEncoderConfig enc_rr_config = {
    .pin_a = PE9,
    .pin_b = PE11,
    .timer = TIM1,
    .inv_dir = false,
    .rad_per_tick = RAD_PER_TICK,
    .frame_id = "rr_wheel_joint",
    .alpha = LOW_PASS_ALPHA,
};

// ───────── Fan and temperature ─────────
inline constexpr NtcConfig ntc_cfg{
    .pin = PB1,
    .pullup_resistance = 5230.0f,
    .c1 = 1.112613927e-03f,
    .c2 = 2.37277392e-04f,
    .c3 = 7.1670e-08f,
    .offset = 273.15 + 3,  // Kelvin to Celsius offset + calibration offset
    .adc_max = 1023.0f};
inline Ntc ntc(ntc_cfg);

inline constexpr uint8_t FAN_PWM_PIN = PB0;

inline constexpr FanConfig fan_config{
    .pin = FAN_PWM_PIN,
    .timer_instance = TIM8,  // ← jawnie, bez auto-detekcji
    .timer_channel = 2,      // CH2 — library wykryje CH2N z PinMap
    .gpio_af = GPIO_AF3_TIM8,
    .complementary = true,  // CH2N = odwrócony sygnał
    .mode = FanMode::Proportional,
    .pwm_frequency = 20000,  // shares TIM8 base with RR motor
    .temp_low = 30,          // poniżej 30°C — duty_min
    .temp_high = 55,         // powyżej 55°C — duty_max
    .duty_min = 20,          // 20% — minimalne obroty (żeby fan nie zatkał)
    .duty_max = 100          // 100% — full blast
};

// ───────── IMU ─────────
inline constexpr ImuBno055Config imu_bno055_config = {
    .bus = &i2c,
    .i2c_addr = 0x29,
    .sensor_id = 0x37,
    .int_pin = PF2,   // ZET6: PF2 (EXTI, GPIO)
    .axis_config = Adafruit_BNO055::REMAP_CONFIG_P1,
    .axis_sign = Adafruit_BNO055::REMAP_SIGN_P2,
};

// ───────── LEDs ─────────
inline constexpr uint8_t RED_LED = PE4;
inline constexpr uint8_t GRN_LED = PE3;

inline constexpr LedIndicatorConfig led_status_config = {
    .pin = RED_LED,
    .initial_state = HIGH,
    .blink_period_ms = 500,
    .label = "STATUS",
};

// ───────── LED Strip ─────────
inline constexpr uint16_t IDLE_ANIMATION_CHANGE_MS = 3500;
inline constexpr uint16_t IDLE_ANIMATION_INTERVAL_MS = 35;
inline constexpr uint16_t LED_STRIP_TIMEOUT_MS = 1000;

// ZET6: SPI3 on PC12/PC11/PC10  [PC12=SPI3_MOSI AF6, PC11=SPI3_MISO AF6, PC10=SPI3_SCK AF6]
inline constexpr SpiTransportConfig spi_config = {
    .mosi_pin = PC12,
    .miso_pin = PC11,
    .sck_pin = PC10,
    .spi_speed = 4000000,
    .bit_order = MSBFIRST,
    .spi_mode = SPI_MODE3,
};

inline constexpr SwapPair swaps[] = {
    {13, 17},
    {14, 16},
};
inline constexpr LedStripConfig strip_config = {
    .num_leds = 18,
    .swaps = swaps,
    .swap_count = sizeof(swaps) / sizeof(swaps[0]),
    .init_r = 0x0F,
    .init_g = 0x00,
    .init_b = 0x00,
};

// ───────── Motors — DRV8874 IN/IN mode ─────────
inline constexpr uint32_t MOTOR_PWM_FREQ = 20000;  // 20 kHz (fan is isolated on TIM8)
inline constexpr float MAX_VELOCITY = 22.0f;
inline constexpr float MIN_VELOCITY = 0.5f;

// ───────── IPROPI current sensing ─────────
// DRV8874 IPROPI: V_sense = I_load × R_sense / K_ipropi
//   K_ipropi = 1200 (typical, per DRV8874PWPR datasheet SLVSER5)
//   R_sense  = 560 Ω  (PCB footprint, 0603, between IPROPI and GND)
//   V_max    = 6A × 560 / 1200 = 2.8 V  < 3.3 V ✓
//   Scale    = V_REF / ADC_MAX / (R_sense / K_ipropi)  [A per ADC count]
inline constexpr float IPROPI_R_SENSE    = 560.0f;   // Ω
inline constexpr float IPROPI_K          = 1200.0f;  // current mirror ratio
inline constexpr float IPROPI_V_REF      = 3.3f;     // V
inline constexpr float IPROPI_ADC_MAX    = 1023.0f;  // 10-bit ADC
inline constexpr float IPROPI_SCALE =
    IPROPI_V_REF / IPROPI_ADC_MAX / (IPROPI_R_SENSE / IPROPI_K);
// IPROPI ADC pins — ADC3 exclusive channels, no conflicts
// PF3(ADC3_IN9), PF4(ADC3_IN14), PF5(ADC3_IN15), PF10(ADC3_IN8)
inline constexpr uint8_t IPROPI_FL = PF3;
inline constexpr uint8_t IPROPI_FR = PF4;
inline constexpr uint8_t IPROPI_RL = PF5;
inline constexpr uint8_t IPROPI_RR = PF10;

// ───────── Software OCP & stall detection ─────────
// These values must match MotorDrv8874Config struct defaults (motor_drv8874.hpp).
// Adjust here for fleet-wide tuning; per-motor overrides go in the motor configs below.
inline constexpr float IPROPI_OC_MAX_CURRENT   = 5.0f;   // [A] hard OCP limit
inline constexpr float IPROPI_STALL_CURRENT    = 3.5f;   // [A] stall detection threshold
inline constexpr float IPROPI_STALL_VEL        = 0.15f;  // [rad/s] stall vel threshold
inline constexpr uint32_t IPROPI_STALL_MS      = 250;    // [ms] stall debounce window

// Driver groups: nSLEEP + nFAULT per side
// ZET6: PG9=nSLEEP_R (GPIO, alt USART6_RX unused), PG10=nSLEEP_L (GPIO)
inline constexpr DriverGroupConfig right_motors_driver = {PG11, PE0};  // PG11: free GPIO (ETH TX stays on PB11/12/13)
inline constexpr DriverGroupConfig left_motors_driver = {PG10, PE1};
inline constexpr DriverGroupConfig driver_groups[] = {
    right_motors_driver,
    left_motors_driver,
};

// ZET6 motor pin assignments (verified against PeripheralPins.c)
// FL: TIM14_CH1 (PF9, AF9) + TIM10_CH1 (PF6, AF3)
inline constexpr MotorDrv8874Config motor_fl_config = {
    .in1_pin = PF9,  // TIM14_CH1 AF9
    .in2_pin = PF6,  // TIM10_CH1 AF3
    .inv_dir = true,
    .max_velocity = MAX_VELOCITY,
    .min_velocity = MIN_VELOCITY,
    .pwm_freq = MOTOR_PWM_FREQ,
    .frame_id = "fl_wheel_joint",
    .ipropi_pin = IPROPI_FL,
    .ipropi_scale = IPROPI_SCALE,
};

// FR: TIM13_CH1 (PF8, AF9) + TIM11_CH1 (PF7, AF3)
inline constexpr MotorDrv8874Config motor_fr_config = {
    .in1_pin = PF8,  // TIM13_CH1 AF9
    .in2_pin = PF7,  // TIM11_CH1 AF3
    .inv_dir = false,
    .max_velocity = MAX_VELOCITY,
    .min_velocity = MIN_VELOCITY,
    .pwm_freq = MOTOR_PWM_FREQ,
    .frame_id = "fr_wheel_joint",
    .ipropi_pin = IPROPI_FR,
    .ipropi_scale = IPROPI_SCALE,
};

// RL: TIM9_CH1 (PE5) + TIM9_CH2 (PE6) — unchanged
inline constexpr MotorDrv8874Config motor_rl_config = {
    .in1_pin = PE5,
    .in2_pin = PE6,
    .inv_dir = true,
    .max_velocity = MAX_VELOCITY,
    .min_velocity = MIN_VELOCITY,
    .pwm_freq = MOTOR_PWM_FREQ,
    .frame_id = "rl_wheel_joint",
    .ipropi_pin = IPROPI_RL,
    .ipropi_scale = IPROPI_SCALE,
};

// RR: TIM8_CH1 (PC6, AF3) + TIM8_CH2 (PC7, AF3)
// Shares TIM8 with Fan (TIM8_CH2N on PB0) — CH1/CH2 and CH2N are independent channels.
// Releases PC9 for I2C3_SDA (EXT2 expansion).
inline constexpr MotorDrv8874Config motor_rr_config = {
    .in1_pin = PC6,  // TIM8_CH1 AF3
    .in2_pin = PC7,  // TIM8_CH2 AF3
    .inv_dir = false,
    .max_velocity = MAX_VELOCITY,
    .min_velocity = MIN_VELOCITY,
    .pwm_freq = MOTOR_PWM_FREQ,
    .frame_id = "rr_wheel_joint",
    .ipropi_pin = IPROPI_RR,
    .ipropi_scale = IPROPI_SCALE,
};

// ───────── PID ─────────
inline constexpr PIDConfig pid_config = {
    .kp = 0.15f,
    .ki = 0.55f,
    .kd = 0.007f,
    .min_output = -1.0f,
    .max_output = 1.0f,
};

// ───────── ROS ─────────
inline constexpr const char* NODE_NAME = "rosbot_pro_mcu";
inline constexpr uint16_t DOMAIN_ID = 255;  // 255 inherit from Micro ROS Agent
inline constexpr uint32_t SPIN_TIME_MS = 1;
inline constexpr uint32_t TIMER_MS = 10;
inline constexpr uint32_t PING_TIMEOUT_MS = 50;
inline constexpr uint8_t PING_ATTEMPTS = 4;

inline byte MAC[6] = {0x02, 0x47, 0x00, 0x00, 0x00, 0x02};
inline IPAddress CLIENT_IP = {192, 168, 77, 3};
inline IPAddress AGENT_IP = {192, 168, 77, 2};
inline uint16_t AGENT_PORT = 8888;

// ───────── Publishers ─────────
inline QueueHandle_t battery_queue;
inline QueueHandle_t imu_queue;
inline QueueHandle_t joint_state_queue;
inline QueueHandle_t led_strip_queue;
inline QueueHandle_t motor_current_queue;
inline QueueHandle_t motor_stall_queue;

inline constexpr uint8_t BATTERY_NUM_CELLS = 3;
inline constexpr float BATTERY_CELL_CAPACITY = 2.6f;  // Ah
inline constexpr float BATTERY_DESIGN_CAPACITY =
    BATTERY_NUM_CELLS * BATTERY_CELL_CAPACITY;
inline constexpr BatteryPublisherConfig battery_pub_config = {
    .topic = "battery",
    .queue = battery_queue,
    .frame_id = "base_link",
    .design_capacity = BATTERY_DESIGN_CAPACITY,
    .num_cells = BATTERY_NUM_CELLS,
};

inline constexpr uint8_t buttons_pins[] = {PUSH_BUTTON1};
inline constexpr ButtonsPublisherConfig buttons_pub_config = {
    .topic = "buttons",
    .pins = buttons_pins,
    .num_buttons = sizeof(buttons_pins) / sizeof(buttons_pins[0]),
};

inline constexpr ImuPublisherConfig imu_pub_config = {
    .topic = "_imu/data",
    .queue = imu_queue,
    .frame_id = "imu_link",
};

inline constexpr JointStatePublisherConfig joint_state_pub_config = {
    .topic = "_motors/feedback",
    .queue = joint_state_queue,
    .frame_id = "base_link",
};

inline constexpr MotorCurrentPublisherConfig motor_current_pub_config = {
    .topic = "_motors/current",
    .queue = motor_current_queue,
};

inline constexpr MotorStallPublisherConfig motor_stall_pub_config = {
    .topic = "_motors/stall",
    .queue = motor_stall_queue,
};

// ───────── Power Board — V1: ADC, V2: Witty Pi 5 ─────────
// V2 WP5 configuration is above (guarded by USE_WITTYPI5_POWER)
// Legacy UART power board config (ROSbot XL) retained for reference:
// inline constexpr PowerBoardConfig power_board_config = {
//     .serial = pb_serial,
//     .baudrate = 38400,
//     .timeout_ms = 100,
//     .v_min = 9.6f,
//     .v_max = 12.6f,
// };

// ───────── SBC Interface ─────────
inline constexpr SerialConfig DIAGNOSTIC_SERIAL_CONFIG = {
    .serial = &Serial1,
    .baudrate = 921600,
    .rxPin = PA10,
    .txPin = PA9,
    .timeout_ms = 1,
    .name = "FTDI_SERIAL"};
